...
import numpy as np
import copy
from networks_attn import Actor, Critic_MATD3
...
class MATD3(nn.Module):
    ...
    def choose_action(self, obs, noise_std, mask=None):
        ...
        obs = torch.unsqueeze(torch.tensor(
            obs, dtype=torch.float), 0).to(device)
        # print(f"{obs.shape = }")
    ...
    def train(self, replay_buffer, agent_n):
        ...
        with torch.no_grad():  # target_Q has no gradient
            ...
            # print(f"{Q1_next.shape = }")
            # print(f"{Q2_next.shape = }")
            # print(f"{len(batch_r_n) = } {batch_r_n[0].shape = }")
            # print(f"{len(batch_done_n) = } {batch_done_n[0].shape = }")
