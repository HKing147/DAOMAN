...
class SelfAttention(nn.Module):
    ...
    def __init__(self, n_embd, n_head, n_agent, masked=False):
        ...
        # regularization
        self.attn_drop = nn.Dropout(0.2)  # 0.2
        self.resid_drop = nn.Dropout(0.2)  # 0.2
        # output projection
        self.proj = init_(nn.Linear(n_embd, n_embd))
...
class MyAttention(nn.Module):
    ...
    def __init__(self, obs_action_dim, n_embd, n_head, n_agent, masked=False):
        super(MyAttention, self).__init__()
        self.obs_encoder = nn.Sequential(nn.LayerNorm(obs_action_dim),
                                         init_(nn.Linear(obs_action_dim, n_embd), activate=True), nn.GELU())

        self.ln = nn.LayerNorm(n_embd)
        self.attn = SelfAttention(
            n_embd=n_embd, n_head=n_head, n_agent=n_agent, masked=masked)
        self.obs_decoder = nn.Sequential(nn.LayerNorm(n_embd),
                                         init_(nn.Linear(n_embd, obs_action_dim), activate=True), nn.GELU())

    def forward(self, x):
        x = self.ln(self.obs_encoder(x))
        x = self.ln(self.attn(x, x, x))
...
class Critic_MATD3(nn.Module):
    def __init__(self, args, attn=None):
        ...
        n_agent = 3
        # self.attn = None
        self.attn = MyAttention(obs_action_dim=args.obs_dim_n[0] +
                                args.action_dim_n[0], n_embd=n_embd, n_head=n_head, n_agent=n_agent, masked=True)
