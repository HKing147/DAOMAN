...
class MATD3(nn.Module):
    ...
    def train(self, replay_buffer, agent_n):
        # 复制前一个agent的模型
        # pre = (self.agent_id-1+3) % 3
        # self.actor.load_state_dict(agent_n[pre].actor.state_dict())
        # self.actor_target.load_state_dict(
        #     agent_n[pre].actor_target.state_dict())
        # self.actor_optimizer.load_state_dict(
        #     agent_n[pre].actor_optimizer.state_dict())
        # self.critic.load_state_dict(agent_n[pre].critic.state_dict())
        # self.critic_target.load_state_dict(
        #     agent_n[pre].critic_target.state_dict())
        # self.critic_optimizer.load_state_dict(
        #     agent_n[pre].critic_optimizer.state_dict())

        self.critic_pointer += 1
        # n_agent, batch, dim
        ...
        # metric = dict()
        # metric[f'critic_loss/agent_{self.agent_id}'] = critic_loss
        self.critic_metrics.append(critic_loss)
        # Optimize the critic
        ...
        if self.critic_pointer % self.policy_update_freq == 0:
            ...
            # metric[f'actor_loss/agent_{self.agent_id}'] = actor_loss
            # metric['actor_loss/actor_pointer'] = self.actor_pointer
            self.actor_metrics.append(actor_loss)
            # Optimize the actor
            ...
            for param, target_param in zip(self.actor.parameters(), self.actor_target.parameters()):
                target_param.data.copy_(
                    self.tau * param.data + (1 - self.tau) * target_param.data)
        # metric['critic_loss/critic_pointer'] = self.critic_pointer
